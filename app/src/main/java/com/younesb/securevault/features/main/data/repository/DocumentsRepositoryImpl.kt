...
class DocumentsRepositoryImpl(
    ...
    override suspend fun createFolder(name: String): FolderDto =
    ...
    override suspend fun deleteFolder(folderId: String, deleteDocuments: Boolean) {
    ...
    override suspend fun getDocuments(): List<DocumentDto> {
        return dao.getDocumentsWithTags().map(DocumentDto::fromDocumentWithTags)
    }

    override suspend fun createDocument(request: DocumentDto) {
        dao.upsertDocument(
            Document(
                id = UUID.randomUUID().toString(),
                name = request.name,
    ...
    override suspend fun renameDocument(documentId: String, newName: String) {
        ...
        dao.upsertDocument(document.copy(
            name = newName,
            updatedAt = System.currentTimeMillis()
        ))
    }

    override suspend fun moveDocument(documentId: String, newFolderId: String?) {
        val document = getDocumentById(documentId)
        dao.upsertDocument(document.copy(
    ...
    override suspend fun addTagToDocument(documentId: String, tagId: String) {
    ...
    override suspend fun removeTagFromDocument(documentId: String, tagId: String) {
